...
def main(conf: Dict,
    ...
    if feature_path is None:
        feature_path = Path(export_dir, file_name)
    feature_path.parent.mkdir(exist_ok=True, parents=True)
    if image_list is not None and os.path.exists(feature_path):
...
def for_query(conf: Dict,
    ...
    dataset = ImageDataset(image_dir, conf['preprocessing'], image_list)
    if feature_path is None:
        feature_path = Path(export_dir, 'query_global_descriptors.h5')
    feature_path.parent.mkdir(exist_ok=True, parents=True)
    skip_names = set(list_h5_names(feature_path)
                     if feature_path.exists() and not overwrite else ())
    dataset.names = [n for n in dataset.names if n not in skip_names]
