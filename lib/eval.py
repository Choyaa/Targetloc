...
import numpy as np
import os
from .transform import qvec2rotmat, rotmat2qvec, convert_quaternion_to_euler, wgs84tocgcs2000
from collections import defaultdict
def evaluate(results, gt, error_path, only_localized=False):
    """
        ...
        for name in test_names:
            if name not in predictions:
                ...
                for s in gt_names:
                    if index in s:
                        gt_name = s
                R_gt, t_gt = gts[gt_name]
                R, t = predictions[name]
                
                q_c2w_gt = rotmat2qvec(R_gt.T)
                euler_gt= np.array(convert_quaternion_to_euler(q_c2w_gt))
                trans_gt = np.array(-R_gt.T.dot(t_gt))  
                
                q_c2w_es = rotmat2qvec(R.T)
                euler_es = np.array(convert_quaternion_to_euler(q_c2w_es))
                trans_es = np.array(-R.T.dot(t)) 
                
                e_R = euler_gt - euler_es
                e_t = trans_gt - trans_es
                qvec = ' '.join(map(str, e_R))
                tvec = ' '.join(map(str, e_t))
                f2.write(f'{gt_name} {qvec} {tvec}\n')
