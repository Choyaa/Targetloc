...
def main(descriptors, num_matched, poses_dict,
         db_descriptors=None,
         ):
    ...
    for i in range(len(pairs)):
        euler_angles_list.append(poses_dict[pairs[i][1]]['euler_angles'])
        translation_list.append(poses_dict[pairs[i][1]]['translation'])
        db_name_list.append(pairs[i][1])
    
    end_time = time.time()
    logger.info("Retrieval cost: %.2f", end_time - start_time)
    return euler_angles_list, translation_list, db_name_list
